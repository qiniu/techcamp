...
编程语言的类型系统可以分为编译时类型系统（静态编译阶段类型检查和类型推导）和运行时类型系统（程序执行阶段管理和操作类型的机制）。
...
XGo 编译器通过 Scanner（词法分析）和 Parser (语法分析) 将 XGo 源代码转换为 XGo AST。

Scanner（词法分析器）是编译器的第一个阶段，负责将 XGo 源码字符流转换为有意义的词法单元（Token）序列，供后续的语法分析器（Parser）使用。

Parser（语法分析器）是编译器的第二个阶段，负责将 Token 序列转换为抽象语法树（AST），检查代码结构是否符合语法规则，并报告语法错误。

- XGo AST => Go AST => Go 源码 转换

通过语法分析得到的 XGo AST 只是保证了语法规则正确，还需要检查语义是否正确，即静态类型检查和表达式类型推导。
这个阶段负责对 XGo AST 的类型进行检查，对表达式类型进行推导，将 XGo AST 转换为 Go AST，并报告语义错误。
...
XGo 编译器在将 XGo AST 转换为 Go AST 过程中需要对类型进行标记和类型检查，确定语义正确性，函数参数是否匹配，接口断言等是否正确，如果出现错误则给出语义错误信息。

XGo 相比 Go 具有更加灵活的表达式类型推导能力，通过静态类型推导，可以将 List/Map 表达式或者 Lambda 表达式推导出实际类型以与 Go 类型系统兼容。
...
对于下面这个例子中 `transform([1, 2, 3], x => x*x)` 首先根据 transform 函数定义推导 `[1, 2, 3]` 类型为 `[]float64`，之后推导 `x` 的类型为 float64，这样这行代码将转换为 Go 语言的 `transform([]float64{1, 2, 3}, func(x float64) { return x*x })`。
...
将 XGo 的 Lambda 表达式与 Go 语言的泛型函数组合可以实现更加强大的语法表达能力。（_注：XGo 本身不支持泛型定义，但可以引用 Go 语言泛型定义。_）

对于下面这个例子中 `transform([1, 2, 3], x => x*x)` 因为 `transform[T]` 为泛型函数，无法确定 `T` 的类型，所以第一轮推导首先推导参数 `[1, 2, 3]` 的类型为 `[]int`，忽略 lambda 表达式类型，之后依据`transform[T]`函数的第一个参数为 `[]int` 对泛型函数 `transform[T]`的泛型参数类型进行推断，确定 `T` 的类型为 `int`，之后对 `transform[T]` 进行实例化为 `transform[int]`，进行第二轮推断，在本轮中根据 `transform[int]` 的第二个参数类型为 `func(int) int`，可推断 lambda 表达式 `x => x*x` 中的 `x` 类型为 `int`。最终这行代码将转换为 Go 语言的 `transform[int]([]int{1, 2, 3}, func(x int) { return x*x })`。
...
而对于 XGo 支持的重载函数的选择需要根据参数个数和参数类型进行类型匹配和类型推导，同样需要进行多轮推断以选择出正确的重载函数或者是匹配失败给出语义错误信息。
